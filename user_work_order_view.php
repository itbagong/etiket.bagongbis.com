...
// ASUMSI: Mendapatkan ID pengguna yang sedang login
$team_member_id = $_SESSION['user_id'] ?? 1;
...
$stmtItem = $db->prepare("
  SELECT
    ...
    s.name AS sub,
    i.qty,
    i.cost -- <<< PERBAIKAN: Mengambil kolom 'cost'
  ...
  <table class="table table-sm table-bordered">
    <thead>
      <tr class="table-secondary">
        <th width="30%">Kategori & Subkategori</th>
        <th width="10%">Quantity</th>
        <th width="60%">Estimasi Biaya (Cost) Item</th>       </tr>
    </thead>
    <tbody>
      <tr>
        ...
        <td>
                    <?php if (isset($item['cost'])): ?>
                        <span class="fw-bold text-success"><?= formatRupiah($item['cost']) ?></span>
                    <?php else: ?>
                        -
                    <?php endif; ?>
                </td>       </tr>
